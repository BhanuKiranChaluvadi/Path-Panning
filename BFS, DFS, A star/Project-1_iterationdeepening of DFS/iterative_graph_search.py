...
class SearchNode:
    def __init__(self, s, actions, parent=None, parent_action=None, cost=0):
        '''
        ...
        #self.g_cost = g_value      ### included G cost foreach node --i willonly using g_cost in uniform,A*
...
class PriorityQ:
    ...
    def push(self, x, cost):
        ...
        if x.state in self.s:
            for y in self.l:
                if ((x.state == y[1].state) and (x.cost < y[1].cost)):      # check for state prsence and cost relative value
    ...
    def pop(self):
        ...
        return x[1]        ## popping the objectouside
...
def dfs(init_state, f, is_goal, actions,iterations):
    for depth in range(3, (iterations+1)):
        frontier = []
        #s = set()
                ...
                if (n_i.cost < depth):
                    for a in actions:
                        ...
                        if((s_prime != n_i.state)):
                            ...
                            if(n_i.cost <= depth):
                                for same_position in frontier:
                                    if(same_position.state == n_prime.state):
                                        if(same_position.cost > n_prime.cost):
...
def bfs(init_state, f, is_goal, actions,steps):
    ...
    while len(stack_of_suspended_vertices)> 0:
        n_i= stack_of_suspended_vertices.pop(0)
        if n_i.state not in visited:
            ...
            if is_goal(n_i.state):
                ...
                for a in actions:
                    ...
                    if(s_prime != n_i.state):
                        if(n_i.cost < steps):
                            itiration = (n_i.cost+1)
                            n_prime= SearchNode(s_prime,actions,n_i,a,itiration)
        ...
        if n_i.state not in visited:
            ...
            if is_goal(n_i.state):
                ...
                for a in actions:
                    ...
                    if(s_prime != n_i.state):
                        cost = (n_i.cost+1)
                        n_prime= SearchNode(s_prime,actions,n_i,a,cost,0)
                ...
                for a in actions:
                    ...
                    if(s_prime != n_i.state):
                        if ( a=='u'or a== 'd'or a=='l'or a=='r'):
                            g_value = n_i.cost+10                     # it cost 10 to move u,d,l,r.
                        else:
                            g_value = n_i.cost+14                     #considering incurred cost to be 14 while moving cross.
                        dx = abs(goal_state[0]-n_i.state[0])
                        dy = abs(goal_state[1]-n_i.state[1])
                        h_value = hypot(dx,dy)
...
def backpath(node,init_pos):
    ...
    path.append(n_0.state)
    # I filled U!!!!
    path = path[::-1]                # reversing the path i.e moving path from initial state to final state.
