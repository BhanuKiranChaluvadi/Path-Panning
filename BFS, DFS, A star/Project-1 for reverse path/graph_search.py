...
_DEBUG_END = True
_ACTIONS = ['r','l','d','u']
_ACTIONS_2 = ['se','sw','nw','ne','r','l','d','u']
_X = 0
_Y = 1
_GOAL_COLOR = 0.75
_INIT_COLOR = 0.25
        ...
        if n_i.state not in visited:
            ...
            if is_goal(n_i.state):
                ...
                for a in actions:
                    ...
                    if(s_prime != n_i.state):         ### It will not explore same node again. visited list had a same check in the top,just wrote it gain.
                        n_prime= SearchNode(s_prime,actions,n_i,a)
                        stack_of_suspended_vertices.append(n_prime)

    print("Path path is not present ")
    return(backpath(n_i,init_state),visited)
